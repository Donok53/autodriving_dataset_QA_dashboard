...
- camera, lidar, imu, gps 센서의 동기화 상태를 요약합니다.
- 급가속, 급제동, GPS jump, 센서 dropout 이벤트를 탐지합니다.
- ROS bag 파일의 토픽 주기, 메시지 수, 센서 커버리지, timestamp gap을 검사합니다.
...
- camera, lidar, imu, gps 계열 토픽 커버리지
- 핵심 센서 누락 여부
- 센서 계열 간 nearest timestamp offset
- IMU 수평 가속도 이벤트
- GPS 위치 jump 이벤트
...
| `camera_timestamp`, `lidar_timestamp`, `imu_timestamp`, `gps_timestamp` | 센서별 timestamp |
| `camera_ok`, `lidar_ok`, `imu_ok`, `gps_ok` | 센서별 수집 상태 |
